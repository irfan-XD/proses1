...
___________________________________________________________________________
 \x1b[1;97mAuthor         : {sir}IRFAN 4444
{x} \x1b[1;97mYour Device    : {sir}Accest Android 10/12
{x} \x1b[1;97mCountry        : {sir}Indonesia 
{x} \x1b[1;97mVersion        : {sir}4.0.2 {x} 
{x} \x1b[1;97mStatus         : {sir} Percobaan {x} ''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tYou User : [green]PREMIUM[white]  '))
		cetak(nel('\tExpired  : [green]20-Agustus-2022[white]  '))
		cetak(nel('\t[red]Warning  [white]: [white]BACA SEBELUM CRACK '))
		cetak(nel('\tKetika Hasil Anda Cp/Tidak Ada Hasil        Ganti Tumbal Anda Karena Cookies Dapat Berpengaruh Pada Saat Crack Berjalan/Berlangsung [[green] banyak berdoa bro!!![white]]  '))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}🍪{x}] \x1b[1;97mMasukkan Cookies Anda :{asu} ')
		...
		print(f'  {x}[{h}✓{x}]{h} Berhasil Login - Jalankan Ulang{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s Login Gagal / Cookies Vailed !!%s'%(x,k,x,m,x))
		exit()
...
def menu(my_name,my_id):
	...
	cetak(nel('\tName Account : [green]%s[white]  '%(my_name)))
	alvino_xy(f' Your ID : '+str(my_id))
	alvino_xy(f' Your Ip  : {ip}')
	cetak(nel('\t             MENU AVAILABLE '))
